...
#include "backend/llvm/emission/control_flow.hpp"
...
static auto control_select_carriers(const Llvm::Representation::Body& body)
    -> Core::Option<const Llvm::Representation::Carriers&> {
  return body.get_program().get_carriers();
}

static auto control_native_builder(const Llvm::Representation::Body& body)
    -> llvm::IRBuilder<>& {
  return *reinterpret_cast<llvm::IRBuilder<>*>(body.get_builder());
}

static auto control_native_function(const Llvm::Representation::Body& body)
    -> llvm::Function& {
...
auto Llvm::Emission::ControlFlow::bind_local(
  ...
  auto carriers = control_select_carriers(body);
  Llvm::Representation::Program& program = body.get_program();
  if (!carriers) {
    return False;
  }
...
auto Llvm::Emission::ControlFlow::constant_local(
    ...
    Ttx::Lexical::Anchor anchor) const -> Bool {
  Llvm::Representation::Program& program = get_program();
  if (program.get_debug().get_level() !=
      Llvm::Representation::Debug::Level::Full) {
    return True;
  }
  ...
  const Llvm::Representation::Carriers& carriers = program.get_carriers();
