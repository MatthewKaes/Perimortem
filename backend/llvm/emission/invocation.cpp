...
#include "backend/llvm/emission/invocation.hpp"
...
static auto call_select_carriers(const Llvm::Representation::Body& body)
    -> Core::Option<const Llvm::Representation::Carriers&> {
  return body.get_program().get_carriers();
}

static auto call_select_functions(const Llvm::Representation::Body& body)
    -> Core::Option<const Llvm::Representation::Functions&> {
  return body.get_program().get_functions();
}

static auto call_fail_backend(
...
static auto call_publish_results(
  ...
  Llvm::Representation::Body::NativeValues values(signature.get_size());
...
auto Llvm::Emission::Invocation::invoke(
  ...
  Llvm::Representation::Body::NativeValues native_arguments(
      inputs.get_size() + 1);
...
auto Llvm::Emission::Invocation::construct(
  ...
  Llvm::Representation::Body& native_body = body;
  Llvm::Representation::Program& native_program = body.get_program();
  const Llvm::Representation::Carriers& carriers =
      native_program.get_carriers();
