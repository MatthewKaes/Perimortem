...
#include "backend/llvm/emission/states.hpp"
...
static auto select_literal_target(Llvm::Representation::Body& body)
...
static auto option_select_carriers(const Llvm::Representation::Body& body)
    -> Core::Option<const Llvm::Representation::Carriers&> {
  return body.get_program().get_carriers();
}

static auto option_native_builder(const Llvm::Representation::Body& body)
    -> llvm::IRBuilder<>& {
  return *reinterpret_cast<llvm::IRBuilder<>*>(body.get_builder());
}

static auto option_native_function(const Llvm::Representation::Body& body)
    -> llvm::Function& {
