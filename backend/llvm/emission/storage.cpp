...
#include "backend/llvm/emission/storage.hpp"
...
static auto sequence_select_carriers(const Llvm::Representation::Body& body)
    -> Core::Option<const Llvm::Representation::Carriers&> {
  return body.get_program().get_carriers();
}

static auto sequence_native_builder(const Llvm::Representation::Body& body)
    -> llvm::IRBuilder<>& {
  return *reinterpret_cast<llvm::IRBuilder<>*>(body.get_builder());
}

static auto sequence_native_function(const Llvm::Representation::Body& body)
    -> llvm::Function& {
  return *llvm::unwrap<llvm::Function>(body.get_function());
}

static auto sequence_find_value(
    const Llvm::Representation::Body& body,
...
auto Llvm::Emission::Storage::range(
  ...
  Llvm::Representation::Body& native_body = body;
...
static auto select_storage_value(Llvm::Representation::Body& body)
    -> Core::Option<StorageSelection> {
  return StorageSelection(
      body, body.get_program(), body.get_program().get_carriers());
}

static auto select_global_storage(const Llvm::Representation::Body& body)
    -> Core::Option<const Llvm::Representation::Globals&> {
...
auto Llvm::Emission::Storage::load(const Ttx::Model::Pack& result) const
    -> Bool {
  auto selected = select_storage_value(body);
  if (!selected) {
    return False;
...
auto Llvm::Emission::Storage::compose(const Ttx::Model::Pack& result) const
    -> Bool {
  Llvm::Representation::Body& native_body = body;
...
static auto select_write_target(Llvm::Representation::Body& body)
