...
#include "backend/llvm/lowering/types.hpp"
...
static auto emit_field(
  ...
  BAIL_IF(!initializer);
  Llvm::Representation::Body native_body(program, field, *initializer);
...
static auto emit_structure(
  ...
  Memory::Dynamic::Vector<Llvm::Representation::Functions::ConstructionField>
      fields;
  for (const Ttx::Concept::Reference<Ttx::Concept::Abstract>& candidate :
       structure.get_addressables()) {
    auto field = candidate.get().select<Field>();
    ...
    BAIL_IF(!value);
    fields.insert(
        Llvm::Representation::Functions::ConstructionField(
