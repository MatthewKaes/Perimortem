...
static auto get_body(Llvm::Representation::Emission& body)
...
static auto get_function(Llvm::Representation::Body& body) -> llvm::Function& {
...
auto Llvm::Representation::Carriers::publish(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type,
...
auto Llvm::Representation::Carriers::reserve(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type,
  ...
  case Kind::Option: {
    Llvm::Abi::Symbol name(
        target->get_arena(), type, Llvm::Abi::Symbol::Kind::OptionType);
  ...
  case Kind::Result: {
    Llvm::Abi::Symbol name(
        target->get_arena(), type, Llvm::Abi::Symbol::Kind::ResultType);
    llvm::StructType& native = *llvm::StructType::create(
        get_context(*target), llvm_text(name.get_view()));
    return publish(
        program, type, Carrier{.kind = kind, .native = llvm::wrap(&native)});
  ...
  case Kind::Structure: {
    Llvm::Abi::Symbol name(
        target->get_arena(), type, Llvm::Abi::Symbol::Kind::StructureType);
    llvm::StructType& native = *llvm::StructType::create(
        get_context(*target), llvm_text(name.get_view()));
    return publish(
        program, type,
  ...
  case Kind::Object: {
    Llvm::Abi::Symbol payload_name(
        target->get_arena(), type, Llvm::Abi::Symbol::Kind::ObjectType);
    llvm::Type& native = *llvm::PointerType::getUnqual(get_context(*target));
...
auto Llvm::Representation::Carriers::begin_completion(
    Llvm::Representation::Emission& program,
...
auto Llvm::Representation::Carriers::select_completion(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type,
...
auto Llvm::Representation::Carriers::complete(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type,
...
auto Llvm::Representation::Carriers::complete_contiguous(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type,
...
auto Llvm::Representation::Carriers::complete_aggregate(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type,
...
auto Llvm::Representation::Carriers::get_type(
...
auto Llvm::Representation::Carriers::get_payload(
    const Ttx::Model::Type& type) const -> Core::Option<LLVMTypeRef> {
  auto found = carriers.find(&type);
...
auto Llvm::Representation::Carriers::get_kind(
...
auto Llvm::Representation::Carriers::get_width(
...
auto Llvm::Representation::Carriers::get_element(const Ttx::Model::Type& type)
    const -> Core::Option<const Ttx::Model::Type&> {
  auto found = carriers.find(&type);
  if (!found || found->value.phase != Phase::Complete ||
      !found->value.element) {
    return {};
...
auto Llvm::Representation::Carriers::get_flag(const Ttx::Model::Type& type)
    const -> Core::Option<const Ttx::Model::Type&> {
  auto found = carriers.find(&type);
...
auto Llvm::Representation::Carriers::get_error(const Ttx::Model::Type& type)
    const -> Core::Option<const Ttx::Model::Type&> {
  auto found = carriers.find(&type);
...
auto Llvm::Representation::Carriers::get_extent(
    const Ttx::Model::Type& type) const -> Core::Option<Count> {
  auto found = carriers.find(&type);
  if (!found || found->value.phase != Phase::Complete ||
...
auto Llvm::Representation::Carriers::get_fields(const Ttx::Model::Type& type)
    const -> Core::Option<const Ttx::Concept::Layout&> {
  auto found = carriers.find(&type);
...
auto Llvm::Representation::Carriers::get_field_index(
...
auto Llvm::Representation::Carriers::get_field_host(
...
auto Llvm::Representation::Carriers::is_real(const Ttx::Model::Type& type) const
    -> Bool {
  auto found = carriers.find(&type);
  return found ? found->value.has(Carrier::Property::Real) : False;
}

auto Llvm::Representation::Carriers::is_signed(
    const Ttx::Model::Type& type) const -> Bool {
  auto found = carriers.find(&type);
  return found ? found->value.has(Carrier::Property::Signed) : False;
}

auto Llvm::Representation::Carriers::is_flag(const Ttx::Model::Type& type) const
    -> Bool {
  auto found = carriers.find(&type);
  return found ? found->value.has(Carrier::Property::Flag) : False;
}

auto Llvm::Representation::Carriers::is_object(
    const Ttx::Model::Type& type) const -> Bool {
  auto found = carriers.find(&type);
  return Bool(found && found->value.kind == Kind::Object);
}

auto Llvm::Representation::Carriers::zero(
    Llvm::Representation::Emission& program,
...
auto Llvm::Representation::Carriers::owns_resources(
    const Ttx::Model::Type& type) const -> Bool {
...
auto Llvm::Representation::Carriers::retain(
...
auto Llvm::Representation::Carriers::release(
    Llvm::Representation::Emission& body,
    const Ttx::Model::Type& type,
    LLVMValueRef value) const -> Bool {
  auto native_body = get_body(body);
...
auto Llvm::Representation::Carriers::select_result(
    Llvm::Representation::Emission& body,
    const Ttx::Model::Type& type,
    LLVMValueRef value,
...
auto Llvm::Representation::Carriers::assemble_result(
    Llvm::Representation::Emission& body,
    const Ttx::Model::Type& type,
    const Ttx::Model::Type& alternative,
    Bool value_selected,
...
auto Llvm::Representation::Carriers::assemble(
    Llvm::Representation::Emission& body,
    const Ttx::Model::Type& type,
    Core::View::Vector<LLVMValueRef> elements) const
    -> Core::Option<LLVMValueRef> {
...
auto Llvm::Representation::Carriers::fit_values(
    Llvm::Representation::Emission& body,
...
auto Llvm::Representation::Carriers::fit_and_assemble(
    Llvm::Representation::Emission& body,
    const Ttx::Model::Type& type,
    const Ttx::Model::Pack& source,
    Core::View::Vector<LLVMValueRef> elements) const
...
auto Llvm::Representation::Carriers::fit(
    Llvm::Representation::Emission& body,
    const Ttx::Model::Pack& source,
    const Ttx::Concept::Layout& target,
    Core::View::Vector<LLVMValueRef> values) const
...
auto Llvm::Representation::Carriers::get_object_descriptor(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type) const -> Core::Option<LLVMValueRef> {
  auto found = carriers.find(&type);
  if (!found || found->value.phase != Phase::Complete) {
  ...
  if (carrier.kind == Kind::ObjectStorage) {
    ...
    if (!carrier.descriptor) {
      ...
      Llvm::Abi::Symbol name(
          target->get_arena(), type, Llvm::Abi::Symbol::Kind::ObjectDescriptor);
  ...
  if (!carrier.finalizer) {
    Llvm::Abi::Symbol name(
        target->get_arena(), type, Llvm::Abi::Symbol::Kind::ObjectFinalizer);
    llvm::FunctionType& signature = *llvm::FunctionType::get(
        llvm::Type::getVoidTy(get_context(*target)),
        {llvm::PointerType::getUnqual(get_context(*target))}, false);
...
auto Llvm::Representation::Carriers::construct(
    Llvm::Representation::Emission& body,
    const Ttx::Model::Type& type,
    Core::View::Vector<LLVMValueRef> values) const
    -> Core::Option<LLVMValueRef> {
