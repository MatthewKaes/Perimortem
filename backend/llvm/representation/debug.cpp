...
auto Llvm::Representation::Debug::release() -> void {
  builder.visit(
      []() {},
...
auto Llvm::Representation::Debug::find_type(const Ttx::Model::Type& type) const
...
auto Llvm::Representation::Debug::publish_type(
...
auto Llvm::Representation::Debug::replace_type(
    const Ttx::Model::Type& type,
    LLVMMetadataRef metadata) -> Bool {
  auto found = types.find(&type);
  if (!found || !metadata) {
...
auto Llvm::Representation::Debug::find_payload(
    const Ttx::Model::Type& type) const -> Core::Option<LLVMMetadataRef> {
  auto found = payloads.find(&type);
  return found ? Core::Option<LLVMMetadataRef>(found->value)
               : Core::Option<LLVMMetadataRef>();
}

auto Llvm::Representation::Debug::publish_payload(
    const Ttx::Model::Type& type,
    LLVMMetadataRef metadata) -> Bool {
...
auto Llvm::Representation::Debug::replace_payload(
    const Ttx::Model::Type& type,
    LLVMMetadataRef metadata) -> Bool {
  auto found = payloads.find(&type);
  if (!found || !metadata) {
...
auto Llvm::Representation::Debug::publish_enumerator(
    const Ttx::Model::Type& type,
    LLVMMetadataRef metadata) -> Bool {
  if (!metadata) {
    return False;
...
auto Llvm::Representation::Debug::get_enumerators(
    const Ttx::Model::Type& type) const -> Core::View::Vector<LLVMMetadataRef> {
  auto found = enumerators.find(&type);
  return found ? found->value.get_view()
               : Core::View::Vector<LLVMMetadataRef>();
}

auto Llvm::Representation::Debug::find_scope(const Ttx::Model::Type& type) const
    -> Core::Option<LLVMMetadataRef> {
...
auto Llvm::Representation::Debug::publish_scope(
    const Ttx::Model::Type& type,
    LLVMMetadataRef metadata) -> Bool {
...
auto Llvm::Representation::Debug::replace_scope(
    const Ttx::Model::Type& type,
    LLVMMetadataRef metadata) -> Bool {
  auto found = scopes.find(&type);
  if (!found || !metadata) {
...
auto Llvm::Representation::Debug::get_scope_types() const
    -> Core::View::Vector<Ttx::Concept::Reference<const Ttx::Model::Type>> {
  return scope_types.get_view();
}

auto Llvm::Representation::Debug::publish_member(
    const Ttx::Model::Type& type,
    LLVMMetadataRef metadata) -> Bool {
  if (!metadata) {
    return False;
...
auto Llvm::Representation::Debug::get_members(
    const Ttx::Model::Type& type) const -> Core::View::Vector<LLVMMetadataRef> {
  auto found = members.find(&type);
  return found ? found->value.get_view()
               : Core::View::Vector<LLVMMetadataRef>();
...
static auto select_program(Llvm::Representation::Emission& program)
...
static auto select_body(Llvm::Representation::Emission& body)
...
static auto native_builder(Llvm::Representation::Program& program)
...
static auto native_file(Llvm::Representation::Program& program)
...
static auto native_scope(Llvm::Representation::Body& body)
...
auto Llvm::Representation::Debug::initialize(
...
static auto native_module(Llvm::Representation::Program& program)
    -> llvm::Module& {
  return *llvm::unwrap(&program.get_module());
}

static auto size_in_bits(
    Llvm::Representation::Program& program,
    llvm::Type& type) -> Count {
...
static auto create_debug_type(
  ...
  auto create = [&](auto& create_type, const Ttx::Model::Type& selected,
    ...
    if (*kind == Llvm::Representation::Carriers::Kind::Value) {
    ...
    if (*kind == Llvm::Representation::Carriers::Kind::Enumeration) {
    ...
    if (*kind == Llvm::Representation::Carriers::Kind::Fixed) {
    ...
    if (*kind == Llvm::Representation::Carriers::Kind::ObjectStorage) {
      auto element = program.get_carriers().get_element(selected);
    ...
    if (*kind == Llvm::Representation::Carriers::Kind::Object) {
    ...
    if (*kind == Llvm::Representation::Carriers::Kind::Option) {
      auto element = program.get_carriers().get_element(selected);
    ...
    Bool structured = *kind == Llvm::Representation::Carriers::Kind::Structure;
    ...
    if (*kind == Llvm::Representation::Carriers::Kind::Option) {
      ...
      members.push_back(&create_member(
          ...
          *debug_flag));
    } else if (*kind == Llvm::Representation::Carriers::Kind::Result) {
      ...
      members.push_back(&create_member(
        ...
        *kind == Llvm::Representation::Carriers::Kind::View ||
        *kind == Llvm::Representation::Carriers::Kind::Access) {
      auto element = program.get_carriers().get_element(selected);
      if (!element) {
        return {};
      }
      ...
      if (*kind == Llvm::Representation::Carriers::Kind::View) {
      ...
      members.push_back(&create_member(
          ...
          count));
    } else if (*kind == Llvm::Representation::Carriers::Kind::Range) {
      auto element = program.get_carriers().get_element(selected);
      if (!element) {
        return {};
      }
      ...
      auto fields = program.get_carriers().get_fields(selected);
      if (*kind != Llvm::Representation::Carriers::Kind::Context && !fields) {
        return {};
      }
...
static auto reserve_debug_scope(
  ...
  if (source && !retained) {
    ...
    if (*kind != Llvm::Representation::Carriers::Kind::Context) {
...
auto Llvm::Representation::Debug::type(
    const Ttx::Model::Type&,
    Core::Option<Ttx::Lexical::Anchor>) -> Bool {
  return True;
}

auto Llvm::Representation::Debug::field(
    const Ttx::Model::Addressable&,
    Ttx::Lexical::Anchor) -> Bool {
  return True;
}

auto Llvm::Representation::Debug::signed_enumerator(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type,
    const Ttx::Concept::Abstract& enumerator,
    S64 value) -> Bool {
  auto selected = select_program(program);
  if (!selected || selected->get_debug().get_level() !=
                       Llvm::Representation::Debug::Level::Full) {
...
auto Llvm::Representation::Debug::unsigned_enumerator(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Type& type,
    const Ttx::Concept::Abstract& enumerator,
    U64 value) -> Bool {
...
auto Llvm::Representation::Debug::global(
    Llvm::Representation::Emission& program,
    const Ttx::Model::Addressable& addressable,
...
auto Llvm::Representation::Debug::begin_function(
...
auto Llvm::Representation::Debug::parameter(
    Llvm::Representation::Emission& body,
...
auto Llvm::Representation::Debug::end_function(
    Llvm::Representation::Emission& body) -> Bool {
  auto selected = select_body(body);
  if (!selected) {
    return False;
  }
...
auto Llvm::Representation::Debug::begin_block(
    Llvm::Representation::Emission& body,
...
auto Llvm::Representation::Debug::statement(
    Llvm::Representation::Emission& body,
    Ttx::Lexical::Anchor anchor) -> Bool {
  auto selected = select_body(body);
...
auto Llvm::Representation::Debug::local(
    Llvm::Representation::Emission& body,
...
auto Llvm::Representation::Debug::value(
    Llvm::Representation::Emission& body,
    const Ttx::Model::Addressable& local,
    Ttx::Lexical::Anchor anchor,
    LLVMValueRef value) -> Bool {
...
auto Llvm::Representation::Debug::finalize(
    Llvm::Representation::Emission& program) -> Bool {
  auto selected = select_program(program);
  if (!selected) {
    return False;
  }
  ...
  for (const Ttx::Concept::Reference<const Ttx::Model::Type>& retained :
    ...
    if (!kind || *kind == Llvm::Representation::Carriers::Kind::Context) {
      auto builder = native_builder(*selected);
