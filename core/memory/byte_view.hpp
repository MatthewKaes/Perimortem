...
// References a managed string, or creates a managed set of bytes in an arena.
//
...
class ByteView {
 public:
  ...
  ByteView(Arena& arena, const std::string_view& source) {
  ...
  ByteView(const std::string_view& source) {
    size = source.size();
    rented_block = source.data();
  }

  // The string is already managed.
  ByteView(const char* source) {
  ...
  // The string is already managed.
  ByteView(const char* source, uint64_t source_size) {
    size = source_size;
    rented_block = source;
  }

  inline constexpr auto operator==(const ByteView& rhs) const -> bool {
  ...
  inline constexpr auto slice(uint64_t start, uint64_t size) const
      -> ByteView {
    return ByteView(rented_block + start, size);
  };
  ...
  inline auto block_compare(const ByteView& data,
