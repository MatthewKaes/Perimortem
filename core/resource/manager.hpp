...
public:
  ...
  struct Resource {
    ...
    inline Resource(const Resource &&rhs)
        : content(std::move(rhs.content)), source(rhs.source), dirty(rhs.dirty),
          time(rhs.time), flags(rhs.flags) {}
  ...
  protected:
    ...
    bool loaded = false;
    Time time;
    Storage::StorageOptionsFlags flags;
  };
  ...
  auto create(const Path &p, Storage::Bytes &&data = Storage::Bytes(),
              Storage::StorageOptionsFlags options =
  ...
  auto import(const Path &p, std::filesystem::path src,
              Storage::StorageOptionsFlags options =
                  Storage::StorageOptions::None) -> Resource *;

  // Set the path to be a real filesystem or a compressed autogenetic
  // filesystem.
  auto config(const Path &p, Storage::StorageOptionsFlags options) -> bool;
...
private:
  struct SectorData {
    auto create(const std::filesystem::path &autogenetic) -> void {
      if (std::filesystem::is_regular_file(autogenetic)) {
        reader = std::make_unique<Storage::VirtualDiskReader>(autogenetic);
        type = reader->get_format();
      } else {
        type = Storage::DiskType::Standard;
      }
    }
