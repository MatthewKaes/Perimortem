...
TEST_F(JsonTests, init_rpc) {
  Arena test;
  uint32_t pos = 0;
  std::string json = load_json("core/storage/tests/json/init_rpc.json");
  ...
  [[maybe_unused]] uint32_t id_value = *(*data)["id"]->get_int();
...
TEST_F(JsonTests, jsonrpc) {
  Arena test;
  uint32_t pos = 0;
  std::string json = load_json("core/storage/tests/json/initialized_rpc.json");
  auto data = parse(test, json, pos);
