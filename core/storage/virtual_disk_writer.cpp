...
template <typename T> auto write_block(Bytes &source, T block) -> void {
...
template <typename T> auto overwrite_block(Byte *source, T block) -> void {
  using stroage_type = T;
  stroage_type data = static_cast<stroage_type>(block);
  if (std::endian::native != file_endian) {
...
auto VirtualDiskWriter::write_resource(const std::string_view &p,
                                       const Byte *data, const SizeBlock size,
                                       StorageOptionsFlags flags) -> void {
  const auto &entry = p;
