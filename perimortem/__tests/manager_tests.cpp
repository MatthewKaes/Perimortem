...
protected:
  virtual void SetUp() {
    ...
    source_root = "perimortem/core/tests/";
    source_root /=
        ::testing::UnitTest::GetInstance()->current_test_info()->name();
...
TEST_F(ManagerTests, single_resource) {
  ...
  auto res = Reader::mount_archive(get_disk(Path::Sector::Resource));
...
TEST_F(ManagerTests, user_resource) {
  ...
  auto usr = Reader::mount_archive(get_disk(Path::Sector::User));
...
TEST_F(ManagerTests, load_from_file) {
  Manager manager("perimortem/tests/simple_project");
  std::filesystem::path svg_path = ".data/icon.svg";
  auto path = Path("[res]://game/images/icon.svg");
