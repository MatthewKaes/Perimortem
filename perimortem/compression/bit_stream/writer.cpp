...
auto Compression::BitStream::Writer::drain() -> void {
  while (bits >= 8) {
    output.append(Unsigned_8(accumulator & 0xFF));
    accumulator >>= 8;
    bits -= 8;
  }
}

auto Compression::BitStream::Writer::write_bits(Unsigned_32 value, Count length)
    -> void {
  if (length == 0) {
    return;
  }

  accumulator |= Unsigned_64(value) << bits;
  bits += length;
  if (bits >= 8) {
    drain();
  }
...
// reversal.
auto Compression::BitStream::Writer::write_code(Unsigned_32 code, Count length)
    -> void {
