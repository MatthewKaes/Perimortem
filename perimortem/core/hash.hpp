...
class Hash {
  ...
  static constexpr auto memcpy_64(
      Unsigned_64 (&result)[chunks],
      const Unsigned_8* data) -> void {
    if consteval {
      for (Count chunk = 0; chunk < chunks; chunk++) {
        result[chunk] = 0;
        result[chunk] |= static_cast<Unsigned_64>(data[7]) << 56;
        result[chunk] |= static_cast<Unsigned_64>(data[6]) << 48;
        result[chunk] |= static_cast<Unsigned_64>(data[5]) << 40;
        result[chunk] |= static_cast<Unsigned_64>(data[4]) << 32;
        result[chunk] |= static_cast<Unsigned_64>(data[3]) << 24;
        result[chunk] |= static_cast<Unsigned_64>(data[2]) << 16;
        result[chunk] |= static_cast<Unsigned_64>(data[1]) << 8;
        result[chunk] |= static_cast<Unsigned_64>(data[0]) << 0;

        data += sizeof(Unsigned_64);
      }
    } else {
      memcpy(&result, data, sizeof(Unsigned_64) * chunks);
    }
  }

  static constexpr auto hash_numeric(Unsigned_64 numeric) {
    Unsigned_64 result = numeric * const_values[0];
 ...
 public:
  constexpr Hash(Core::View::Bytes bytes) {
    const Unsigned_8* data = bytes.get_data();
    ...
    Unsigned_64 small_hash_value = 0;
    switch (bytes.get_size()) {
    case 7:
      small_hash_value |= static_cast<Unsigned_64>(data[6]) << 48;
    case 6:
      small_hash_value |= static_cast<Unsigned_64>(data[5]) << 40;
    case 5:
      small_hash_value |= static_cast<Unsigned_64>(data[4]) << 32;
    case 4:
      small_hash_value |= static_cast<Unsigned_64>(data[3]) << 24;
    case 3:
      small_hash_value |= static_cast<Unsigned_64>(data[2]) << 16;
    case 2:
      small_hash_value |= static_cast<Unsigned_64>(data[1]) << 8;
    case 1: {
      small_hash_value |= static_cast<Unsigned_64>(data[0]);
    ...
    Unsigned_64 result[block_stride];
    const Count block_count = bytes.get_size() / sizeof(Unsigned_64);
    ...
    for (Count processed = 0; processed + block_stride <= block_count;
      ...
      // block chunks.
      Unsigned_64 block[block_stride];
      memcpy_64(block, data);
      data += sizeof(Unsigned_64) * block_stride;
    ...
    Count left_over =
        bytes.get_size() & (sizeof(Unsigned_64) * block_stride - 1);
    switch (left_over) {
    ...
    case 9: {
      ...
      Unsigned_64 remainder_tail[1];
      memcpy_64(remainder_tail, data + left_over - sizeof(Unsigned_64));

      // Shift out the extra bites
      remainder_tail[0] >>= (sizeof(Unsigned_64) * 2 - left_over) * 8;
    ...
    case 1: {
      // Read the exact 8 bytes left over.
      Unsigned_64 remainder[1];
      memcpy_64(remainder, data + left_over - sizeof(Unsigned_64));

      // Shift out the extra bites
      remainder[0] >>= (sizeof(Unsigned_64) - left_over) * 8;
    ...
    Unsigned_64 final_result = result[0];
    for (Count i = 0; i < block_stride; i++) {
  ...
  constexpr Hash(Signed_8 numeric)
      : value(hash_numeric(Unsigned_64(numeric))) {}
  constexpr Hash(Signed_16 numeric)
      : value(hash_numeric(Unsigned_64(numeric))) {}
  constexpr Hash(Signed_32 numeric)
      : value(hash_numeric(Unsigned_64(numeric))) {}
  constexpr Hash(Signed_64 numeric)
      : value(hash_numeric(Unsigned_64(numeric))) {}
  constexpr Hash(Unsigned_8 numeric)
      : value(hash_numeric(Unsigned_64(numeric))) {}
  constexpr Hash(Unsigned_16 numeric)
      : value(hash_numeric(Unsigned_64(numeric))) {}
  constexpr Hash(Unsigned_32 numeric)
      : value(hash_numeric(Unsigned_64(numeric))) {}
  constexpr Hash(Unsigned_64 numeric)
      : value(hash_numeric(Unsigned_64(numeric))) {}
  template <typename pointer_type>
  Hash(pointer_type* pointer)
      : value(hash_numeric(Unsigned_64(reinterpret_cast<CppSize>(pointer)))) {}
  ...
  constexpr auto Rehash(Unsigned_64 hash) const -> Unsigned_64 {
    Unsigned_64 result;
 ...
 private:
  ...
  static constexpr Unsigned_64 const_values[5] = {
