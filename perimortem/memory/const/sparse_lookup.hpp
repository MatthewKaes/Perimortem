...
constexpr Count default_table_seed = 0x506572696d6f7274;
...
template <typename value_type,
          ...
          Count sparse_factor = 2'00,
          Count sparse_seed = default_table_seed>
class SparseLookupTable {
 ...
 private:
  ...
  // perfect hashes.
  inline static constexpr Count hash_string(const Byte* str,
                                               Count length) {
    constexpr auto initial_state = (0xcbf29ce484222325 ^ sparse_seed) *
                                   static_cast<Count>(0x100000001b3ull);
    if (length == 0)
      return initial_state;

    Count d = initial_state;
    for (Count i = 0; i < length; i++)
      d = (d ^ static_cast<Count>(str[i])) *
          static_cast<Count>(0x100000001b3ull);
    return d;
  }
  ...
  inline static constexpr auto calculate_container_size(Count value)
      -> Count {
  ...
  inline static constexpr auto create_sparse_table() -> const
    ...
    for (Count i = 0; i < element_count; i++) {
      ...
      for (Count check = 0; check < element_count; check++) {
 ...
 public:
  ...
  static constexpr Count table_size =
      calculate_container_size(element_count);
  ...
  static constexpr auto find_or_default(const Byte* data,
                                        value_type default_value)
      -> value_type {
    Count size = std::char_traits<char>::length(data);
  ...
  static constexpr auto find_or_default(const Byte* data,
    ...
    Count hash = hash_string(data, size);
    ...
    if (no_collisions) {
      ...
      for (Count check = 0; check < max_checks(); check++) {
  ...
  static constexpr auto max_checks() -> Count {
    Count max = 0;
  ...
  static constexpr auto has_perfect_hash(Count threshold = 1) -> bool {
    return max_checks() <= threshold;
  }

  static constexpr auto has_broken_hash() -> bool { return max_checks() == -1; }

  static constexpr auto check_counts() -> std::array<Count, element_count> {
    std::array<Count, element_count> counts = {};
    for (Count i = 0; i < element_count; i++) {
 ...
 private:
  static constexpr auto check_count(const Byte* data) -> Count {
    Count size = std::char_traits<char>::length(data);
    return check_count(data, size);
  }

  static constexpr auto check_count(const Byte* data, Count size)
      -> Count {
    Count hash = hash_string(data, size);
    for (Count check = 0; check < element_count; check++) {
      const auto& entry = sparse_table[(hash + check) % table_size];
...
template <Count search_depth,
          typename value_type,
          Count element_count,
          const TablePair<const Byte*, value_type> (&source)[element_count],
          Count sparse_factor = 2'00,
          Count sparse_seed = default_table_seed,
          Count collision_threshold = 1>
class SeedFinder {
 public:
  // static_assert(false,
  ...
  inline static constexpr auto candidate_search() -> Count {
...
class SeedFinder<0,
                 ...
                 collision_threshold> {
 public:
  inline static constexpr auto candidate_search() -> Count { return 0; }
};
