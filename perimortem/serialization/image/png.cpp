...
#include "perimortem/serialization/image/png.hpp"
...
using namespace Perimortem::Serialization::Image;
...
static auto channels_for_color_type(Png::ColorType color_type) -> Count {
  switch (color_type) {
...
static auto convert_to_pixels(
  ...
  Count source_channels = channels_for_color_type(color_type);
...
auto Png::read_info(const View::Bytes source) -> ImageInfo {
  ...
  if (!ihdr.valid || ihdr.type.get_view() != "IHDR"_view ||
      ihdr.length != ihdr_data_size) [[unlikely]] {
    return ImageInfo();
  }
  ...
  if (channels_for_color_type(info.color_type) == 0) [[unlikely]] {
    return ImageInfo();
  }
...
auto Png::decode(const View::Bytes source) -> Graphics::Image {
  ...
  if (idat_combined.get_size() == 0) [[unlikely]] {
    return Graphics::Image();
  }

  // Inflate: decompress the zlib-wrapped DEFLATE stream from the IDAT chunks.
  Dynamic::Bytes filtered_rows = Compression::inflate(idat_combined.get_view());
  if (filtered_rows.get_size() == 0) [[unlikely]] {
    return Graphics::Image();
  }

  Count bytes_per_pixel = channels_for_color_type(info.color_type);
...
auto Png::encode(const Graphics::Image& image) -> Dynamic::Bytes {
  ...
  for (Count row = 0; row < Count(height); row++) {
