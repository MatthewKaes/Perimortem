...
auto Writer::create(Type format_, CompressionLevels compression_) -> void {
  format = format_;
...
auto Writer::stage_resource(const Memory::View::Bytes p,
...
auto Writer::write_disk(const std::filesystem::path& p) -> bool {
...
auto Writer::compress(Managed::Bytes& output, View::Bytes source) -> bool {
  const int level =
