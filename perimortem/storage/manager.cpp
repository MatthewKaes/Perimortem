...
#include "perimortem/storage/manager.hpp"
#include "perimortem/storage/archive/writer.hpp"
...
auto Manager::set_sector_type(Path::Sector sector, Storage::Type type) -> void {
...
auto Manager::flush_changes() -> void {
  ...
  Writer vtables[Path::sector_count];
  ...
  for (const auto& entry : directory) {
    ...
    Writer& table = vtables[(int32_t)entry.first.get_sector()];
    table.stage_resource(entry.first.get_origin(), content.data(),
...
auto Manager::SectorData::create(const std::filesystem::path& autogenetic)
  ...
  reader = Storage::Reader::mount_archive(autogenetic);
