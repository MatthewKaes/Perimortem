...
#include "perimortem/storage/serialization/binary/stream.hpp"

#include "perimortem/core/data_model.hpp"

using namespace Perimortem::Memory;
using namespace Perimortem::Storage::Serialization::Binary;
...
template <typename storage_type>
constexpr auto write_block(Dynamic::Bytes& data,
                           Count& ptr_location,
                           storage_type bin,
                           Stream::DataType type) -> void {
  data.resize(ptr_location + sizeof(Stream::DataType) + sizeof(Bits_16));
  data.get_data()[ptr_location] = static_cast<Byte>(type);

  bin = possible_swap(bin);

  Perimortem::Core::copy(data.get_data() + ptr_location + 1, &bin);
...
auto Stream::operator<<(const Bits_8 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::Bits_8);
  return *this;
}

auto Stream::operator<<(const Bits_16 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::Bits_16);
  return *this;
}

auto Stream::operator<<(const Bits_32 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::Bits_32);
  return *this;
}

auto Stream::operator<<(const Bits_64 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::Bits_64);
  return *this;
}

auto Stream::operator<<(const SignedBits_8 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::SignedBits_8);
  return *this;
}

auto Stream::operator<<(const SignedBits_16 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::SignedBits_16);
  return *this;
}

auto Stream::operator<<(const SignedBits_32 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::SignedBits_32);
  return *this;
}

auto Stream::operator<<(const SignedBits_64 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::SignedBits_64);
  return *this;
}

auto Stream::operator<<(const Real_32 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::Real_32);
  return *this;
}

auto Stream::operator<<(const Real_64 bin) -> Stream& {
  write_block(data, ptr_location, bin, DataType::Real_64);
  return *this;
}

auto Stream::operator<<(const View::Bytes bin) -> Stream& {
  data.resize(ptr_location + sizeof(DataType) + sizeof(Count) + bin.get_size());
  data.get_data()[ptr_location++] = static_cast<Byte>(DataType::Blob);

  Count size = possible_swap(bin.get_size());
  Perimortem::Core::copy(data.get_data() + ptr_location, &size);
  ptr_location += sizeof(Count);

  Perimortem::Core::copy(data.get_data() + ptr_location, bin.get_data(), bin.get_size());
