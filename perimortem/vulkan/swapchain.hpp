...
class Swapchain {
 public:
  static auto create(const Context& ctx, Unsigned_32 width, Unsigned_32 height)
      -> Swapchain;
  ...
  auto recreate(const Context& ctx, Unsigned_32 width, Unsigned_32 height)
      -> void;
  ...
  auto acquire_next_image(VkSemaphore signal_semaphore) const -> Unsigned_32;

  auto get_swapchain() const -> VkSwapchainKHR;
  auto get_extent() const -> VkExtent2D;
  auto get_format() const -> VkFormat;
  auto get_image(Unsigned_32 index) const -> VkImage;
  auto get_image_view(Unsigned_32 index) const -> VkImageView;
  auto get_image_count() const -> Unsigned_32;

  static constexpr Unsigned_32 max_images = 8;
