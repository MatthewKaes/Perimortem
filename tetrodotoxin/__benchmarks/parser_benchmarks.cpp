...
static_assert(
    sizeof(lookup::sparse_table) <= 600,
...
static void OPTIMIZED_STL_vector_table(benchmark::State& state) {
  ...
  for (int i = 0; i < KeywordTable::lookup::item_count; i++) {
    ...
    vector_table[index].push_back(
        std::make_pair(
            std::string_view{KeywordTable::data[i].key, index + 1},
...
using collision_lookup = SparseLookupTable<
    KeywordTable::value_type,
...
static void sparse_lookup_table_collision(benchmark::State& state) {
  for (auto _ : state) {
    for (int i = 1; i <= tests; i++) {
      auto entry = test_data[rand() % test_data.size()];
      doNotOptimizeAway(
          collision_lookup::find_or_default(
...
static void sparse_lookup_table_perfect_stack(benchmark::State& state) {
  for (auto _ : state) {
    for (int i = 1; i <= tests; i++) {
      auto entry = test_data[rand() % test_data.size()];
      doNotOptimizeAway(
          KeywordTable::lookup::find_or_default(
...
using index_lookup = SparseIndexTable<
    KeywordTable::value_type,
    array_size(KeywordTable::data),
    KeywordTable::data>;
...
static inline constexpr auto check_keyword(
  ...
  static_assert(
      sizeof(keyword_table::sparse_table) <= 4800,
