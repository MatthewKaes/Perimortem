...
auto App::Archive::Writer::write(
  ...
  if (windowed) {
    ...
    if (auto width = windowed->get_width()) {
      Appender(output) << *width;
    }
    if (auto height = windowed->get_height()) {
      Appender(output) << *height;
    }
    if (auto resizable = windowed->get_resizable()) {
      Appender(output) << U8(*resizable ? 1 : 0);
    }
  }
  ...
  Appender(output) << U8(program ? 0 : 1);
  if (program) {
    BAIL_IF(
  ...
  Appender(output) << U32(scene->get_transitions().get_size());
  for (const Reference<App::Language::Transition>& retained :
    ...
    Appender(output) << U8(transition.get_action());
    const auto& destination = transition.get_destination_route();
    Appender(output) << U8(destination ? 1 : 0);
    if (destination) {
