...
static auto parse_source(Perimortem::Core::View::Bytes source_path) -> Bool {
  ...
  Ttx::Parse::Source source = Ttx::Parse::Source::parse(cursor, source_path);
...
Signed_32 main(Signed_32 argc, Signed_8** argv) {
  ...
  // TODO: this is the temporary terminal edge while the TTX pipeline is being
  // rebuilt from the source envelope upward. The CLI now proves that sources
  // and dependencies parse through the shared TTX envelope, but the package
  // graph, dialect body parsing, and real terminal generation still need to be
  // connected before generated headers can publish `Ttx::...` declarations.
