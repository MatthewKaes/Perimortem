...
static auto parse_reachable_roots(
    Cursor& cursor,
    Managed::Vector<Environment::ReachableRoot>& roots) -> Bool {
  BAIL_IF(!cursor.require(
  ...
  while (!cursor.matches(Code::Type::BracketEnd)) {
    if (cursor.matches(Code::Type::String)) {
      ...
      BAIL_IF(
          ...
          (*locator)[0] == '\\');
      roots.insert({
        .kind = Environment::ReachableRoot::Kind::Project,
        .locator = *locator,
      });
    } else {
      BAIL_IF(!require_spelling(
      ...
      roots.insert({
        .kind = Environment::ReachableRoot::Kind::Sdk,
        .locator = "sdk"_view,
      });
    }
...
auto Environment::Dialect::parse_region(Cursor& cursor) const -> Option<Plan&> {
  Managed::Vector<Environment::ReachableRoot> reachable_roots(
      cursor.get_arena());
