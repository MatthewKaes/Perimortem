...
auto Language::Access::Instance::can_bind(const Abstract& binding) const
    -> Bool {
...
auto Language::Access::Instance::bind(Abstract& binding, Bool published)
    -> Bool {
...
auto Language::Access::Instance::is_published(const Abstract& binding) const
    -> Bool {
  auto selected = bindings.find(binding.get_name());
  return selected && &selected->value.semantic.get() == &binding &&
...
auto Language::Access::Instance::resolve_published(Core::View::Bytes name) const
    -> const Abstract& {
  auto selected = bindings.find(name);
...
auto Language::Access::Instance::resolve_concept(Core::View::Bytes name) const
    -> const Abstract& {
  auto selected = bindings.find(name);
  if (selected) {
    return selected->value.semantic.get();
...
auto Language::Access::Instance::visit_concepts(
