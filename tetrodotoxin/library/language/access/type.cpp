...
static auto follow_alias(const Abstract& selected) -> const Abstract& {
  return selected.visit<Ttx::Model::Alias>(
      [](const Ttx::Model::Alias& alias) -> const Abstract& {
        return follow_alias(alias.get_target());
      },
      [](const Abstract& exact) -> const Abstract& { return exact; });
}
...
auto Language::Access::Type::resolve(const Abstract& context) const
    -> const Abstract& {
  const Abstract& exact_context = follow_alias(context);
...
auto Language::Access::Type::resolve_from(const Abstract& root) const
    -> const Abstract& {
  Reference<const Abstract> selected(follow_alias(root));
  Count segment_start = root_size;
  ...
  while (segment_start < route.get_size()) {
    ...
    const Abstract& next = selected.get().resolve_context(segment);
    selected = Reference<const Abstract>(follow_alias(next));
