...
static auto report_types(
  ...
  report << "Greater cannot use left Type `"_view << left.get_type().get_name()
...
static auto report_fold_error(
    Cursor& cursor,
    Span span,
    const Language::Operations::Greater& greater,
    const Language::Expression& left,
  ...
  case Language::FoldError::Type::InvalidOperationType:
    if (&error.get_expression() == &greater) {
  ...
  auto report = cursor.create_report(span);
  report << "Greater input `"_view << error.get_expression().get_name()
...
auto Language::Operations::Greater::parse(
  ...
  const auto& greater = domain.construct<Greater>(domain, left, *right);
  if (!greater.get_type().resolve().is<Ttx::Model::Types::Flag>()) {
    report_types(transaction, span, left, *right);
    return {};
  }

  auto folded = greater.attempt_fold(domain, materializations);
  auto parsed = folded.visit(
      ...
      [&](const FoldError& error) -> Utility::Option<const Expression&> {
        report_fold_error(transaction, span, greater, left, *right, error);
        return {};
      });
  if (!parsed) {
    return {};
...
auto Language::Operations::Greater::get_type() const -> const Abstract& {
