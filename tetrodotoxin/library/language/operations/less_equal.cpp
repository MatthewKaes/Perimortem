...
static auto report_types(
  ...
  report << "LessEqual cannot use left Type `"_view
         << left.get_type().get_name() << "` with right Type `"_view
         << right.get_type().get_name() << "`."_view;
  report.get_hint()
...
static auto report_fold_error(
    Cursor& cursor,
    Span span,
    const Language::Operations::LessEqual& less_equal,
    const Language::Expression& left,
  ...
  case Language::FoldError::Type::InvalidOperationType:
    if (&error.get_expression() == &less_equal) {
  ...
  auto report = cursor.create_report(span);
  report << "LessEqual input `"_view << error.get_expression().get_name()
...
auto Language::Operations::LessEqual::parse(
  ...
  if (!right) {
    transaction.create_expression_error(
        span, "LessEqual has a malformed right operand."_view,
        "Use a complete scalar Expression after `<=`."_view);
    return {};
  }

  const auto& less_equal = domain.construct<LessEqual>(domain, left, *right);
  if (!less_equal.get_type().resolve().is<Ttx::Model::Types::Flag>()) {
    report_types(transaction, span, left, *right);
    return {};
  }

  auto folded = less_equal.attempt_fold(domain, materializations);
  auto parsed = folded.visit(
      ...
      [&](const FoldError& error) -> Utility::Option<const Expression&> {
        report_fold_error(transaction, span, less_equal, left, *right, error);
        return {};
      });
  if (!parsed) {
    return {};
  ...
  // grammar, numeric legality, and eager folding complete successfully.
...
auto Language::Operations::LessEqual::get_type() const -> const Abstract& {
