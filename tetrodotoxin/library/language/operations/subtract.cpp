...
static auto report_types(
  ...
  report << "Subtract cannot use left Type `"_view << left.get_type().get_name()
...
static auto report_overflow(
    Cursor& cursor,
    Span span,
    const Language::Operations::Subtract& subtract,
    const Language::Expression& left,
    const Language::Expression& right) -> void {
  auto report = cursor.create_report(span);
  report << "Subtract cannot represent "_view;
      ...
      [&](const Language::Constants::Signed& left_value) {
        right.visit<Language::Constants::Signed>(
            [&](const Language::Constants::Signed& right_value) {
              report << left_value.get_value() << " minus "_view
  ...
  report << " in selected Type `"_view << subtract.get_type().get_name()
         << "`."_view;
  report.get_hint()
      << "Use values whose difference fits the selected Type width."_view;
}
...
static auto report_fold_error(
  ...
  case Language::FoldError::Type::InvalidOperationType:
    if (&error.get_expression() == &subtract) {
  ...
  case Language::FoldError::Type::ArithmeticOverflow:
    if (&error.get_expression() == &subtract) {
      report_overflow(cursor, span, subtract, left, right);
      return;
    }
    break;
  default:
  ...
  auto report = cursor.create_report(span);
  report << "Subtract input `"_view << error.get_expression().get_name()
...
auto Language::Operations::Subtract::parse(
  ...
  const auto& subtract = domain.construct<Subtract>(domain, left, *right);
  if (!subtract.get_type().resolve().is<Type>()) {
    report_types(cursor, span, left, *right);
    return {};
  }

  auto folded = subtract.attempt_fold(domain, materializations);
  return folded.visit(
      [](const Expression& expression) -> Utility::Option<const Expression&> {
        return expression;
      },
      [&](const FoldError& error) -> Utility::Option<const Expression&> {
        report_fold_error(cursor, span, subtract, left, *right, error);
        return {};
      });
}
...
auto Language::Operations::Subtract::get_type() const -> const Abstract& {
  return result_type;
}
