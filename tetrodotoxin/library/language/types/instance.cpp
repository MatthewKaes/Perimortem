...
auto Types::Instance::can_bind(const Abstract& binding) const -> Bool {
  ...
  return !selected || selected->value.semantic == &binding;
}
...
auto Types::Instance::resolve_published(Core::View::Bytes name) const
  ...
  if (selected && selected->value.published) {
    return *selected->value.semantic;
  }
...
auto Types::Instance::visit_concepts(ttx_named_abstract_callable* visitor) const
    -> void {
  for (Count entry = 0; entry < bindings.get_size(); ++entry) {
    ...
    if (selected != nullptr && selected->value.published) {
      const Abstract& semantic = *selected->value.semantic;
      visit_concept(visitor, semantic.get_name(), semantic);
    }
  }
}
