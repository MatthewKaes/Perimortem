...
auto Types::Instance::can_bind(const Abstract& binding) const -> Bool {
...
auto Types::Instance::bind(Abstract& binding, Bool published) -> Bool {
...
auto Types::Instance::is_published(const Abstract& binding) const -> Bool {
  auto selected = bindings.find(binding.get_name());
  return selected && &selected->value.semantic.get() == &binding &&
         selected->value.published;
}

auto Types::Instance::resolve_published(Core::View::Bytes name) const
    -> const Abstract& {
  auto selected = bindings.find(name);
...
auto Types::Instance::resolve_concept(Core::View::Bytes name) const
    -> const Abstract& {
  auto selected = bindings.find(name);
  if (selected) {
    return selected->value.semantic.get();
...
auto Types::Instance::get_concepts(Context& context) const -> const Pack& {
  Surface surface(*this);
  return context.pack(surface);
}

auto Types::Instance::Surface::select(Count index) const -> const Binding* {
...
auto Types::Instance::Surface::get_size() const -> Count {
  Count size = 0;
  for (Count entry = 0; entry < owner.bindings.get_size(); entry++) {
    const auto* selected = owner.bindings.get_entry(entry);
    if (selected != nullptr && selected->value.published) {
...
auto Types::Instance::Surface::get_abstract(Count index) const
...
auto Types::Instance::Surface::get_name(Count index) const
...
auto Types::Instance::Surface::fits_entry(
    const Layout& target,
    Count source_index,
...
auto Types::Instance::Surface::fits_at(
    const Layout& target,
...
auto Types::Instance::Surface::get_fitted_at(
    const Layout& target,
    Count target_offset,
