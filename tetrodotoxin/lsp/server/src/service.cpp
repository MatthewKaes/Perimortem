...
auto recursive_strip(
    const Tetrodotoxin::Lexical::Tokenizer::Tokens stream,
    uint32_t& index) -> void {
...
auto Service::lsp_tokens(Tokenizer& tokenizer, const RpcRequest& header)
  ...
  Managed::Vector<Core::View::Bytes> imports(header.get_arena());
  Managed::Vector<Core::View::Bytes> parameters(header.get_arena());
  uint32_t scopes = 0;
    ...
    case Classifier::Identifier:
      if (i < tokens.get_size() - 2 &&
        ...
        token_data.insert("Fu"_view);
          ...
          i > 0 && tokens[i - 1].klass != Classifier::AccessOp &&
