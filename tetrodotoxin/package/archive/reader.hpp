...
class Reader {
 public:
  ...
  // Reads one complete Format 1 envelope. Rejection logs the exact validation
  // stage and retains nothing. The caller that knows why this Archive was
  // requested decides whether failure becomes a textual source diagnostic.
  // Success borrows the input and retains its record ranges in the caller
  // Arena. The caller keeps the input valid until that Arena is reset or
