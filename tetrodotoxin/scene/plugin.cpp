...
class ScenePluginError final : public Tetrodotoxin::Language::Error {
 public:
  TTX_NAME("Scene plugin error"_view);

  void describe(Ttx::Lexical::Errors::Report& report) const override {
    report << "Scene plugin could not acquire its Library dependency."_view;
  }
};

static constexpr uint8_t package_name[] = "Tetrodotoxin.Scene";
...
static constexpr uint8_t library_route[] = "Dialect";
...
static const ttx_requirement_descriptor dependencies[] = {
  {
    ...
    .exported_route = {library_route, sizeof(library_route) - 1},
...
static ScenePluginError error;
static std::unique_ptr<Tetrodotoxin::Scene::Dialect> dialect;
...
extern "C" TTX_EXPORT void TTX_CALL
  ...
  auto cpp = Tetrodotoxin::Plugin::resolve_requirement(host, dependencies[1]);
  if (!library || !cpp) {
    result.operations->failed(result.self, error.get_abi());
    return;
  }
  Ttx::Abstract* local = Ttx::Abstract::local_owner(*library, *cpp);
  if (local == nullptr) {
    result.operations->failed(result.self, error.get_abi());
    return;
  }
  auto& semantic = static_cast<Ttx::Concept::Abstract&>(*local);
  auto& dependency = static_cast<Tetrodotoxin::Library::Dialect&>(semantic);
  dialect = std::make_unique<Tetrodotoxin::Scene::Dialect>(dependency);
