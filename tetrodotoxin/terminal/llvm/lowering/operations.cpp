...
static auto lower_arithmetic(
  ...
  auto carrier = operation.get_type().resolve().select<Ttx::Model::Domain>();
  BAIL_IF(
...
static auto lower_comparison(
  ...
  auto carrier = left.get_type().resolve().select<Ttx::Model::Domain>();
...
auto Llvm::Lowering::Operations::lower(
  ...
  if (negate) {
    ...
    auto carrier = negate->get_type().resolve().select<Ttx::Model::Domain>();
