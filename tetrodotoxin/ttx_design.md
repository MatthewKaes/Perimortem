...
| `PascalCase`  | types, builtins, aliases, and package-level type paths      |
...
Keywords and reserved names are tokenized before the parser sees them. For
example, `Enum`, `Struct`, `Foreign`, `func`, and `@if` are distinct token
classes, not ordinary identifiers that the parser has to reinterpret later.
However all reserved keywords all bucket into ther lexical forms as well:
- `Enum`, `Struct`, and `Foreign` are also "Types"
- `func` creates addressable functions
- `@if` is a compiler directive for a conditional disable (`\>`)
...
To help with later layers, each TTX file starts with exactly one package declaration
...
The package line has no sigil. That keeps the top of the file visually distinct
and gives the parser a stable entry point. Parsing can start below package
level, but a full source file is anchored by `package : Dialect;`.

`Library`, `Shader`, and `Entity` are package dialects. A dialect is more than
a backend name: it decides which builtins, types, attributes, and lowering rules
are legal in that package. For example, `package : Shader;` does not support
managed runtime types such as `Object` or `List`, because those concepts do not
exist in the shader execution model.

A dialect may produce more than one output. A `Shader` package produces shader
code, such as SPIR-V embedded as binary constants, and host code, such as
x86_64 code that loads those constants, builds the required `Layout` values, and
bridges them into the shader pipeline.
...
```ttx
import Alias : PackageKind = source;
```
...
general source language. The definition type tells the parser what kind of node
is being created.

The active package dialect may reject otherwise valid builtin forms. `Object`
...
The small vector types are provided by the implicit `TTX.Core` prelude as
structs with concrete `Real_32` fields:
...
| `.[start +: count]` | slice into a positional pack              |
...
Line comments start with `//`. Consecutive comments before a member are treated
as documentation for that member by the syntax tree and formatter:
