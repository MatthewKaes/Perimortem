...
template <typename Owner, typename Operations>
static auto select_owner(const Operations* operations) -> Owner& {
  static_assert(offsetof(Owner, operations) == 0);
  return *reinterpret_cast<Owner*>(const_cast<Operations*>(operations));
}

static auto select_context(ttx_context self) -> ContextOwner& {
  ContextOwner& selected = select_owner<ContextOwner>(self.operations);
  if (selected.authority != self.owner || self.value != 1) {
    std::abort();
  }
  return selected;
}
...
static auto select_pack(ttx_pack self) -> PackOwner& {
  PackOwner& selected = select_owner<PackOwner>(self.operations);
  if (selected.authority != self.owner || selected.value != self.value) {
    std::abort();
  }
  return selected;
}
...
static auto select_snapshot(ttx_layout_snapshot_result self)
    -> SnapshotCapture& {
  return select_owner<SnapshotCapture>(self.operations);
}

static auto select_enumerable(ttx_enumerable_result self)
    -> EnumerableCapture& {
  return select_owner<EnumerableCapture>(self.operations);
}

static auto select_validation(ttx_layout_entry_sink self) -> EntryValidation& {
  return select_owner<EntryValidation>(self.operations);
}
...
static void TTX_CALL validate_entry(
  ...
  if (domain.state == Ttx::Observation::Resolved &&
    ...
    const ttx_enumerable_result result = {
      ...
      .owner = domain.layout.owner,
      .value = domain.layout.value,
    };
...
static auto validate_snapshot(ttx_layout_snapshot snapshot) -> bool {
  ...
  // Exact cardinality closes the distinction between empty flow and a Layout
  // that merely failed to visit its unsettled positions. Producer identities
  // remain borrowed from the graph and are never copied into Context policy.
...
static void TTX_CALL context_pack(
  ...
  PackOwner* pack = new (std::nothrow) PackOwner(
      context.authority, context.packs, capture.snapshot, context.next_pack);
  if (pack == nullptr) {
    release_snapshot(capture.snapshot);
...
ContextOwner::ContextOwner()
    : operations({
      ...
      packs(nullptr),
      authority(ttx_authority_create()),
      next_pack(1) {}
