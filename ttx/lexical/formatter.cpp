...
class State {
 public:
  State(const Tokenizer& tokenizer)
 ...
 private:
  ...
  auto classify(Count first, Count last, ScopeRole scope) const -> Section {
    ...
    if (unit_start < last && has_scope &&
        ...
        tokens[unit_start].caculate_text(source) == "foreign"_view) {
  ...
  auto name_precedes(const Unit& left, const Unit& right) const -> Bool {
    ...
    View::Bytes left_name = tokens[left.name].caculate_text(source);
    View::Bytes right_name = tokens[right.name].caculate_text(source);
    Count index = 0;
