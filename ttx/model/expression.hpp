...
// required to evaluate the expression. The active Dialect owns parsing,
// operator legality, and executable bodies while shared fitting and reflection
// consume these stable queries.
class Expression : public Concept::Abstract {
 public:
  ...
  virtual constexpr auto get_inputs() const -> const Concept::Layout& = 0;
  ...
  virtual constexpr auto fits(const Type& target) const -> Bool {
