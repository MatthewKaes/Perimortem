...
//   EXPECT_TEXT(member_type(*color, "green"_view)->get_name(),
//   "Unsigned_8"_view); const auto* red_facts =
//   record->get_implementation().find(*red); ASSERT(red_facts != nullptr);
//   EXPECT(red_facts->get_modifier() == Ttx::Lexical::Code::Type::Expose);
...
//   EXPECT_TEXT(member_type(*filter, "none"_view)->get_name(),
//   "Unsigned_8"_view); const Ttx::Function* default_filter =
...
//   ASSERT(definition != nullptr);
//   EXPECT(definition->get_modifier() == Ttx::Lexical::Code::Type::Expose);
...
//   ASSERT_EQ(body->count<Tetrodotoxin::Compiler::Execution::Call>(),
//   Count(2));
//   ASSERT_EQ(body->count<Tetrodotoxin::Compiler::Execution::Return>(),
//   Count(1));
...
//   ASSERT(body != nullptr);
//   EXPECT_EQ(body->count<Tetrodotoxin::Compiler::Execution::Binary>(),
//   Count(0)); ASSERT_EQ(body->get_operands().get_size(), Count(1)); const
//   auto* constant =
//       body->get_operands()[0]
//           .find<Tetrodotoxin::Compiler::Execution::Constant>();
...
//   EXPECT_TEXT(
//       source_context.get_errors()[0].get_hint(), "Did you mean
//       `second`?"_view);
...
//   color->find_attribute("shader_type"_view); ASSERT(attribute != nullptr);
