...
class LessEqualUnresolvedType : public Ttx::Model::Domain {
 public:
...
PERIMORTEM_UNIT_TEST(LibraryLessEqual, type_selection) {
  ...
  LessEqualExpression invalid("invalid"_view, Unknown::get_unknown());
...
PERIMORTEM_UNIT_TEST(LibraryLessEqual, ieee_domains) {
  ...
  auto wide_yes = selected(test_fold(wide_true));
  auto wide_no = selected(test_fold(wide_false));
  auto wide_equal_result = selected(test_fold(wide_same));
  ...
  EXPECT(wide_yes->is_identity<Constants::True>());
  EXPECT(wide_no->is_identity<Constants::False>());
  EXPECT(wide_equal_result->is_identity<Constants::True>());
  EXPECT(positive_result->is_identity<Constants::False>());
  EXPECT(negative_result->is_identity<Constants::True>());
...
PERIMORTEM_UNIT_TEST(LibraryLessEqual, atomic_provenance) {
  ...
  EXPECT(less_equal.get_type().resolve().is<Unknown>());
  EXPECT(link_operation(less_equal, source));
  EXPECT(link_operation(less_equal, source));
  EXPECT(link_operation(failure, source));

  auto first = selected(test_fold(less_equal));
  auto second = selected(test_fold(less_equal));
  ...
  auto parsed = Interpreter::Operation::parse_binary(
      Code::Type::LessEqOp, source, success_cursor, success_left,
  ...
  auto rejected = Interpreter::Operation::parse_binary(
      Code::Type::LessEqOp, source, failure_cursor, failure_left,
      Span(failure_left_token));

  ASSERT(parsed);
  EXPECT(parsed->is_identity<Operations::LessEqual>());
  ...
  ASSERT(rejected);
  EXPECT(rejected->is_identity<Operations::LessEqual>());
