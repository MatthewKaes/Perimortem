...
//   return build_archive_buffer(resolver, *package, "User.Callable"_view,
//   output);
// }
...
//   const Resolution::Source::Record* core =
//   consumer.resolve("User.Core"_view); ASSERT(app != nullptr); ASSERT(ui !=
//   nullptr); ASSERT(core != nullptr);
...
//   EXPECT(image->find_addressable_function("get_size_pixels"_view) !=
//   nullptr); EXPECT(image->find_addressable_function("sample"_view) !=
//   nullptr); const Ttx::Type* size = graphics_type->find_type("Size2D"_view);
//   ASSERT(size != nullptr);
//   EXPECT_TEXT(
...
//   ASSERT(body != nullptr);
//   ASSERT_EQ(body->count<Tetrodotoxin::Compiler::Execution::Call>(),
//   Count(1)); EXPECT_TEXT(
//       body->find<Tetrodotoxin::Compiler::Execution::Call>()->get_symbol(),
//       "print"_view);

//   const Resolution::Source::Record* host =
//   resolver.resolve("User.Host"_view); ASSERT(host != nullptr); const
//   Ttx::Type* console = host->get_type().find_type("Console"_view);
...
//   ASSERT(body != nullptr);
//   ASSERT_EQ(body->count<Tetrodotoxin::Compiler::Execution::Call>(),
//   Count(1)); View::Bytes call_symbol =
//       body->find<Tetrodotoxin::Compiler::Execution::Call>()->get_symbol();
...
//   EXPECT_NOT(resolver.register_package_buffer(
//       conflict_context, "user_core_conflict.puffer"_view,
//       actual_core_buffer));
...
//   Ttx::Type root("Root"_view, View::Vector<Ttx::Member>(),
//   nested.get_view()); View::Bytes output =
...
//   }};
//   View::Bytes output =
//       write_test_package(writer_arena, "User.Terminal"_view, root,
//       terminals);
