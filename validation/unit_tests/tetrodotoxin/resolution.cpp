...
// static auto import_name(const Resolution::Source::Record* record, Count
// index)
//     -> View::Bytes {
...
//       "dialect : Library;\n"
//       "bad source with enough text to notice if it is copied per
//       error\n"_view;
...
//   // A memory source is not cached until its imports resolve, so A cannot
//   appear
...
//   // The failed A load left no cache record, so B cannot close the cycle
//   through
//   // a stale in-memory A.
//   EXPECT_NOT(resolver.load_source(
...
//   // Once B is valid, A can depend on the cached record even though B is not
//   on
//   // disk.
//   Resolution::Resolver::Context b_source_context;
//   ASSERT(resolver.load_source(
...
//   // Breaking dep1 removes every consumer that points into it, but dep3
//   survives
...
//   // Package imports resolve through the public package name, while the
//   private
...
//     Resolution::Resolver resolver(isa_registry, {},
//     failures[i].package_name); Resolution::Resolver::Context source_context;
//     EXPECT_NOT(resolver.load_source(
